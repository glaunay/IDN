...
,
		...
		helpLink : {
			"experiment"  : {
								...
								content : '<div class="helpMeContent"><i class="fa fa-hand-o-right"></i> additional help can be found <a href = "http://www.youtube.com/watch?v=_QB
WC2-GSgE" target = "_blank">Here</a></div>',
							 	html  : true,
							 	placement : "bottom",
			...
			"association" : {
								...
								content : '<div class="helpMeContent"><i class="fa fa-hand-o-right"></i> additional help can be found <a href = "https://www.youtube.com/playlist?l
ist=PLw61Ua8TujDRLFSZMZChqmqX7yFBYLaz0" target = "_blank">Here</a></div>',
							 	html  : true,
							 	placement : "bottom",
							 	container : "div#reportDiv",
							 	trigger : "manual"
			...
			"publication" : {
								...
								content : '<div class="helpMeContent"><i class="fa fa-hand-o-right"></i> additional help can be found <a href = "http://www.youtube.com/watch?v=ziA
4O8mfGrw" target = "_blank">Here</a></div>',
							 	html  : true,
							 	placement : "bottom",
							 	container : "div#reportDiv",
							 	trigger : "manual"
			...
			"biomolecule" : {
								...
								content : '<div class="helpMeContent"><i class="fa fa-hand-o-right"></i> additional help can be found <a href = "http://www.youtube.com/watch?v=yT8
pglPVR3w" target = "_blank">Here</a></div>',
							 	html  : true,
							 	placement : "bottom",
							 	container : "div#reportDiv",
							 	trigger : "manual"
 ...
 */
	...
	psicquicViewHotPatch : function () {
		var scaffold = '<div id="psicquicView" class="row-fluid"><div class="span2 psqLogo"><img class="pull-left" src="' + this.rootUrl + 
			...
			+ '<div class="psqStatus span8"><div class="row-fluid"><div class="psqTitle span12">Psicquic remote querying</div><div class="span12">Expanding to partners found in PSICQUIC-enabled datab
ases</div></div></div>' 
			...
			if (this.jsonData.xref){
				...
				.forEach(function(elem){
					     ...
					     if(elem.hasOwnProperty('Molecule_Processing')) {
						 name = elem.Molecule_Processing;					
						 if (dataRoot.relationship.Belongs_to) {
						     name = dataRoot.relationship.Belongs_to[0] + '-' + name;
						 }
					     }
...
');
  		...
  		_draw : function(){//dessine les composant en fonction du type de data
		    ...
		    $(self.targetDomElem).find("span.addCart.biom")
			.click(function(){
				   self.addCartCallback({type : "biomolecule" , value : $( this ).attr("name")})
			       });
		    $(self.targetDomElem).find("span.addCart.kewrd")
			.click(function(){
				   self.addCartCallback({type : "keyword" , value : $( this ).attr("name")})
			       });
		    $(self.targetDomElem)
			.find("span.addCart.publi")
			.click(function(){
				   console.dir(this);
		    ...
		    $(self.targetDomElem).find("nav div.navigueBar a").click(function() {
										...
										$(self.targetDomElem).find("nav div.navigueBar a").css({"background-color" : "black"});
										$(self.targetDomElem).find(this).css({"background-color" : "#6495ED"});
										$(nomPage).css({"background-color" : "black"});
									     });
 ...
 */
  		...
  		_externalRefGenerateDivHtml : function(barchart){
  			...
  			if(self._nameAccess()){content += "<dt class ='hReport'>Uniprot reference:</dt><dd><a target = '_blank' href = 'http://www.uniprot.org/uniprot/" + self.jsonData.name + "'>" + self.jsonDat
a.name + "</a></dd>";}
  				...
  				for (var i=0; i < self.jsonData.xref.length; i++) {
			 		$.each(self.jsonData.xref[i], function(name,id){
			 			if(name=="Molecule_Processing"){
			 				content += "<dt class ='hReport'>" + mapper[name].term +":</dt><dd><a target = '_blank' href = '" + mapper[name].url + self.jsonData.relationship.Belongs_t
o[0] + '#' +id + "'>" + id +"</a></dd>"
			 			}else{
		...
		_specie : function(data){
			...
			if(data){
				...
				var specieString ="<dt class ='hReport'>Species:</dt><dd> <a target = '_blank' href = 'http://www.ncbi.nlm.nih.gov/Taxonomy/Browser/wwwtax.cgi?id=" + self.jsonData.specie.value + 
"'>" + nom + " <i class='fa fa-external-link'></i></a></dd>";
		...
		_domainAnnot : function(){
			...
			if(self.jsonData.pfam){
				domainString +='<dt><a href="http://pfam.sanger.ac.uk/" target = "_blank"><img src="../../img/pfamLogo.png" alt="pfamLogo" style="border:none;width:40px;" /></a></dt><dd><a target
 = "_blank" href = "http://pfam.sanger.ac.uk/family/' + self.jsonData.pfam.data[0].id + '">' + self.jsonData.pfam.data[0].Desc + '</a>' ;
				...
				domainString +='<dt><a href="https://www.ebi.ac.uk/interpro/" target = "_blank"><img src="../../img/logoInterpro.png" alt="interproLogo" style="border:none;height:20px;margin-top:
4px;" /></a></dt><dd><a target = "_blank" href = "https://www.ebi.ac.uk/interpro/entry/' + self.jsonData.interpro.data[0].id + '">' + self.jsonData.interpro.data[0].EntryName + '</a>'
				for (var i = 1; i < self.jsonData.interpro.data.length; i++) {
			  		domainString += ', <a target = "_blank" href = "https://www.ebi.ac.uk/interpro/entry/' + self.jsonData.interpro.data[i].id + '">' + self.jsonData.interpro.data[i].EntryNam
e + '</a>';
				};
		...
		_pdbList : function(){
			...
			for (var i=0; i < self.jsonData.pdb.data.length; i++) {
			  pdbString = pdbString + "<li><div class='row-fluid'><div class = 'span4'> " + self.jsonData.pdb.data[i].id +"</div><div class='span4'>" + self.jsonData.pdb.data[i].determinationMethod +
 "</div><i class='fa fa-eye pdbView' id ='" + self.jsonData.pdb.data[i].id + "'></i></li>"
			};
 ...
 */
		...
		_assoLink : function(){
			...
			for (var i=0; i < self.jsonData.association.length; i++) {
			 returnString+= '<dd><a target = "_blank" href ="' + this.rootUrl + '/cgi-bin/current/newPort?type=association&value=' + self.jsonData.association[i] + '">' + self.jsonData.association[i]
 + "</a></dd>";
			};
			return returnString;
		},
...
[deniaudm@ariane js]$ cat report.js
/*
 traitement des différent report 
 affichage résultat +  navigation top
 initialisation:
		...
		_publication : function(){
			...
			if (self.jsonData.publication instanceof Array) {
				...
				for (var i=0; i < self.jsonData.publication.length; i++) {
					...
					+ self.jsonData.publication[i].name + '">'
 ...
 */
  		_interactionOrganisator : function(){// organise le bandeau intéraction
			...
			data.aaData[i][1] ='<span index = "' + i + '">'+ data.aaData[i][1] + '</span> <i class="fa fa-info-circle"></i>';
  		    ...
  		    var titre = "<div class='divTitre'> <div class='interactBannerSup'>" 
			+ this.jsonData.name 
			+ " has <span class = 'niceRed'>" 
  			+ data.aaData.length + "</span> partner" + plural + ' '
			+ ' described in ' + (data.xpTotal.inferred  + data.xpTotal.genuine) + 
			' experiments:</div><div class="interactBannerBot"><ul class="xpPool">'
			+ '<li><span class="headGenCount">' 
  			+ data.xpTotal.genuine + '</span> involving ' + this.jsonData.name + '</li>'
			+ '<li><span class="headInfCount">' 
			+ data.xpTotal.inferred + '</span> involving one of its homologs </li></ul></div>';
...
//			if(returnString){returnString+= self._assocPartner() + "</dl></div>";}
		...
		_supportByAssoc : function(string){
		    ...
		    if(string){
			returnString = string;
			//    var returnString = string + '<dt > Supported by interaction:</dt>';
		    } else {
			returnString = '<div class ="divTitre">Interaction of homologous ' 
			    + 'biomolecule(s)</div><div class = "postitContent">';
//			    + '<dl><dt class = "hReport"> Supported by interaction:</dt>';
		...
		if(string) {
		    ...
		    returnString = '<div class ="divTitre"> Inferred human interaction </div>'
...
*/	
		_uniProtWord : function(){
			...
			$(self.targetDomElem).append("<div class ='author Content contentXp content'><div class = 'keywrdDef'>"+
  			...
  			var ancre = " <div id = 'nomPage'  style = 'margin-left:20px;' class = 'navigueBar'><a>" + self.jsonData.identifier + " " + self.jsonData.name + "</a></div>";
